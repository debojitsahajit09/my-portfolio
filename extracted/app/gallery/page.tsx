...
export default function Gallery() {
  ...
  // 'All' ক্যাটাগরির জন্য সব ছবি একসাথে ফ্ল্যাট করা
  const allImages = Object.values(galleryData)
    .flat()
        ...
        {selectedCategory === "All" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {allImages.map((img: any, idx: number) => (
              <GalleryCard key={img.id || idx} img={img} />
            ))}
          </div>
        ) : (
          <div className="space-y-12">
            ...
            {selectedCategory === "Academic" && (
              <>
                <SubSection
                  ...
                  images={galleryData.ndc || []}
                />
                <SubSection
                  title="Rajbari Govt. High School"
                  images={galleryData.rghs || []}
                />
                <SubSection
                  title="Cambridge School and College"
                  images={galleryData.cambridge || []}
                />
              </>
            )}
            ...
            {selectedCategory === "Graphics Design" && (
              <SubSection
                title="Graphics Design, Banners & Posters"
                images={galleryData.graphics || []}
              />
            )}
            ...
            {selectedCategory === "Projects" && (
              <SubSection
                title="Projects & Technical Developments"
                images={galleryData.projects || []}
              />
            )}
            ...
            {selectedCategory === "Myself" && (
              <SubSection
                title="Personal Moments & Portraits"
                images={galleryData.myself || []}
              />
            )}
          </div>
        )}
